...
class ProjectStatus(models.Model):
    ...
    group = models.IntegerField(
        null=True, blank=True,
        help_text=('To group different status with different level, give them '
...
class Activity(models.Model):
    """
    Represents a work activity (Example: Earth work, concrete works,
    finishing works, electrical works, sanitary works ...)
    """
    title = models.CharField(
        max_length=100,
        help_text=('Type of the work activity. Example: Concrete works, '
                   'Electrical works, Finishing works..')
    )
    description = models.TextField(null=True, blank=True)
...
class Variation(models.Model):
    ...
    title = models.CharField(
        max_length=100,
        help_text='Title for the variation works',
    )
    description = models.TextField(
        null=True, blank=True,
        help_text='Short description about the variation works and/or work order. (Optional)',
    )
    work_order = models.CharField(
        max_length=10,
        null=True, blank=True,
        help_text='Work order no. of the variation works. (Optional)',
    )
    received_date = models.DateField(
        help_text='Received date of the variation or work order. (Use yyyy-mm-dd format)',
    )
    received_letter = models.CharField(
        'Ref. No. for received Order (Optional)',
        max_length=100,
        null=True, blank=True,
        help_text='Ref. No. for the letter containing the order received.',
    )
    submitted_date = models.DateField(
        null=True, blank=True,
        help_text='Submitted date of the variation. (Use yyyy-mm-dd format)',
    )
    submitted_letter = models.CharField(
        'Ref. No. for submission letter (Optional)',
        max_length=100,
        null=True, blank=True,
        help_text='Ref. No. for the submission letter of the variation.',
    )
    submitted_amount = models.DecimalField(
        max_digits=14, decimal_places=2,
        null=True, blank=True,
        help_text='Submitted amount in ETB',
    )
    approved_date = models.DateField(
        'Approved/Rejected Date',
        null=True, blank=True,
        help_text='Approved date of the variation. (Use yyyy-mm-dd format)',
    )
    approved_letter = models.CharField(
        'Ref. No. for approval/rejection letter (Optional)',
        max_length=100,
        null=True, blank=True,
        help_text='Ref. No. for the approval or rejection letter.',
    )
    approved_amount = models.DecimalField(
        max_digits=14, decimal_places=2,
        null=True, blank=True,
        help_text='Approved amount in ETB',
    )
    remark = models.TextField(
        null=True, blank=True,
        help_text='Short helpfull remarks or notes regarding the current status. (Optional)',
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def get_absolute_url(self):
        return reverse('dashboard:variation-detail', args=[str(self.pk)])
...
class Claim(models.Model):
    ...
    title = models.CharField(
        max_length=100,
        help_text='Title for the Time Extension Claim',
    )
    description = models.TextField(
        null=True, blank=True,
        help_text='Short description about the time extension and claimant arguments. (Optional)',
    )
    submitted_date = models.DateField(
        null=True, blank=True,
        help_text='Submitted date of the time extension request. (Use yyyy-mm-dd format)',
    )
    submitted_letter = models.CharField(
        'Ref. No. for submission letter (Optional)',
        max_length=100,
        null=True, blank=True,
        help_text='Ref. No. for the submission letter of the time extension request.',
    )
    submitted_amount = models.IntegerField(
        'Requested Amount in Calendar Days',
        help_text='Submitted amount in calendar days',
    )
    approved_date = models.DateField(
        'Approved/Rejected Date',
        null=True, blank=True,
        help_text='Approved date of the time extension request. (Use yyyy-mm-dd format)',
    )
    approved_letter = models.CharField(
        'Ref. No. for approval/rejection letter (Optional)',
        max_length=100,
    ...
    approved_amount = models.IntegerField(
        'Approved Amount in Calendar Days',
        null=True, blank=True,
        help_text='Approved amount in calendar days',
    )
    remark = models.TextField(
        null=True, blank=True,
        help_text='Short helpfull remarks or notes regarding the current status. (Optional)',
    ...
    def get_absolute_url(self):
        return reverse('dashboard:claim-detail', args=[str(self.pk)])
...
class InsuranceType(models.Model):
    """
    Represents an insurance bond type (Example: Advance bond guranttee,
    Performance bond guranttee)
    """
    title = models.CharField(
        max_length=100,
        help_text=('Type of Insurance Bong Guranttee. Example: Advance bond '
                   'guranttee, Performance bond guranttee etc..')
    )
    short_title = models.CharField(
        max_length=60,
        help_text='Shorter title for mobile responsive views',
    )
    description = models.TextField(null=True, blank=True)
...
class Bank(models.Model):
    ...
    full_name = models.CharField(
        max_length=100,
        help_text='Full official name of the bank or insurance provider firm',
    )
    short_name = models.CharField(
        max_length=100,
        help_text='Short (unofficial) commonly used name of the bank or insurance provider firm',
    )
    description = models.TextField(
        'Short description (optional)',
        null=True, blank=True,
        help_text='Short optional description about the firm',
    )
...
class Insurance(models.Model):
    ...
    amount = models.DecimalField(
        max_digits=16, decimal_places=2,
        null=True, blank=True,
        help_text='Insurance amount in ETB',
    )
    start_date = models.DateField(
        help_text='Starting date of this insurance. (Use yyyy-mm-dd format)',
    )
    period = models.IntegerField(
        help_text='Number of calendar days covered by the insurance',
    )
    end_date = models.DateField(
        'Expiration date',
        null=True, blank=True,
        help_text='Expiration date of this insurance. (Use yyyy-mm-dd format)')
    status = models.ForeignKey(InsuranceStatus, on_delete=models.CASCADE)
    issue_number = models.IntegerField(
        'Number of revision',
        null=True, blank=True,
        help_text='If unsure, please leave it blank',
    )
    description = models.TextField(
        'Note (optional)',
        null=True, blank=True,
        help_text='Optional notes or remarks regarding the insruance',
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def get_absolute_url(self):
        return reverse('dashboard:insurance-detail', args=[str(self.pk)])
